      ...
      <div className='flex justify-between p-3 items-center'>
        ...
        <div className='hidden lg:block'>
          <ul className='flex items-center gap-6'>
            {NavbarMenu.map((item) => (
              <li key={item.id}>
                <Link to={item.link} className='text-gray-900 text-xl xl:text-base py-1 px-2 xl:px-3 hover:text-amber-50'>
                  {item.title}
                </Link>
              </li>
            ))}
        ...
        <div className='hidden lg:block space-x-6'>
          <div className='flex items-center gap-7'>
            ...
            <IoIosNotifications
              className='text-pink-600 text-3xl cursor-pointer hover:text-amber-50'
